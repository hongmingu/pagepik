$(function () {
    if ($('#search_word').html() === '') {
        ...
        $.ajax({
            url: '/re/search/my/', type: 'post', dataType: 'json', cache: false,
            data: {
    ...
    $('#more_load').on('click', function (e) {
        e.preventDefault()
        $.ajax({
            url: '/re/search/mine/', type: 'post', dataType: 'json', cache: false,
            data: {
                search_word: $('#search_word').html(),
                end_id: $('#end_id').html()
            },
            success: function (data) {
                if (data.res === 1) {
                    ...
                    if (data.output.length === 0) {
                        ...
                        var keyword = ''
                        $.each(value.keyword_output, function (key, value) {
                            keyword = keyword + '<span class="search_suobj_keyword div_base">' + value + '</span>'
                        })
